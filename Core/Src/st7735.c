...
void ST7735_Init(void) {
  ...
  ST7735_SetRotation(ST7735_ROTATION); // 设置旋转角度
  ST7735_FillScreen(ST7735_BLACK);
}

void ST7735_SetAddressWindow(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1) // 设置地址窗口
{
  x0 += ST7735_XSTART;
  y0 += ST7735_YSTART;
...
void ST7735_DrawRectangle(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t color) // 绘制矩形
{
...
void ST7735_DrawChar(uint16_t x, uint16_t y, char c, uint16_t color, uint16_t bgColor, const FontDef *font) // 绘制字符
{
  uint32_t i, b, j;
  ...
  // 对于 12x6 字体（6x12），使用与 Smart_Card 项目完全一致的渲染方式
...
void ST7735_DrawString(uint16_t x, uint16_t y, const char *str, uint16_t color, uint16_t bgColor, const FontDef *font) // 绘制字符串
{
  while (*str) {
...
/**
  ...
  */
void ST7735_FillRect(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t color) // 填充矩形区域
{
  ST7735_DrawRectangle(x, y, width, height, color);
}

void ST7735_DrawImage(uint16_t x, uint16_t y, uint16_t width, uint16_t height, const uint8_t *image) // 绘制图像
{
  ST7735_SetAddressWindow(x, y, x + width - 1, y + height - 1);
