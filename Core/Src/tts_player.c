...
#define TTS_TEST_TONE_HALF_CYCLE 16U
...
static void TTS_Player_ResetState(void)
{
  g_ttsBusy = false;
...
{
  (void)TTS_Player_PlayTestTone();
  while (TTS_Player_IsBusy()) {
  }
}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  if (g_ttsTestToneMode) {
    sample = (((g_ttsIndex / TTS_TEST_TONE_HALF_CYCLE) % 2U) == 0U) ? 3600U : 496U;
